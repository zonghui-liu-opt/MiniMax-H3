...
from test_infer_smoke_8gpu import servers
...
class RefInferenceTest(unittest.TestCase):
    ...
    def run_on(self, instances, *extra):
        argv = ["--metadata", str(self.metadata), "--output-dir", str(self.output),
                ...
                "--allow-unmarked-server", "--poll-interval", ".001", *extra]
        with mock.patch.object(ref, "validate_runtime"):
    ...
    def test_payload_names_unicode_and_explicit_seed(self):
        ...
        self.assertEqual([c["role"] for c in request["conditions"]], ["keyframe", "reference", "reference"])
        ...
        self.assertTrue(request["conditions"][2]["uri"].startswith("data:video/mp4;base64,"))
        ...
        self.assertIn("<omitted>", preview["conditions"][2]["uri"])
    ...
    def test_server_commands_select_ref2va_and_unique_ports(self):
        ...
        for _, command in commands:
            ...
            self.assertEqual(command[command.index("--served-model-name") + 1], "minimax-h3-ref2va")
